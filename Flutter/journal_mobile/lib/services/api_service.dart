...
class CacheKeys {
  ...
  static String getScheduleCacheKey(
    String accountId,
    String dateFrom,
    String dateTo,
...
class ApiService {
  ...
  Future<String?> login(String username, String password) async {
    return await _executeWithLimit(_getRequestKey('login'), () async {
  ...
  Future<Account> loginAndCreateAccount(
    String username,
    String password,
  ) async {
  ...
  Future<List<Mark>> getMarks(String token) async {
    return await _executeWithLimit(_getRequestKey('marks'), () async {
      ...
      try {
        ...
        final cachedData = await _cacheRepository.get(
          CacheKeys.getMarksCacheKey(accountId),
          accountId: accountId,
        );
        if (cachedData is List) {
          final cachedMarks = cachedData
  ...
  Future<UserData> getUser(String token) async {
    return await _executeWithLimit(_getRequestKey('user'), () async {
      final accountId = await _getCurrentAccountId();
  ...
  Future<List<ScheduleElement>> getSchedule(
    ...
    return await _executeWithLimit(
      ...
      () async {
        final accountId = await _getCurrentAccountId();
        final cacheKey = CacheKeys.getScheduleCacheKey(
          accountId,
          dateFrom,
          dateTo,
        );
        ...
        try {
          ...
          if (response.statusCode == 200) {
            ...
            final schedule = scheduleData
                .map(
                  (json) =>
                      ScheduleElement.fromJson(json as Map<String, dynamic>),
                )
                .toList();
            ...
            print(
              '✅ Расписание загружено и сохранено в SQLite: ${schedule.length} шт',
            );
            ...
            final offlineSchedule = await _databaseFacade
                .getScheduleByDateRange(accountId, rangeStart, rangeEnd);
            if (offlineSchedule.isNotEmpty) {
              print(
                '📱 Используем расписание из SQLite: ${offlineSchedule.length} шт',
              );
          ...
          // Пробуем из кэша
          final cachedSchedule = await _cacheRepository
              .get<List<ScheduleElement>>(cacheKey, accountId: accountId);

          if (cachedSchedule != null && cachedSchedule.isNotEmpty) {
            print(
              '💾 Используем расписание из кэша: ${cachedSchedule.length} шт',
            );
          ...
          if (offlineSchedule.isNotEmpty) {
            print(
              '🗄️ Используем расписание из SQLite: ${offlineSchedule.length} шт',
            );
            return offlineSchedule;
          }
  ...
  Future<List<LeaderboardUser>> getGroupLeaders(String token) async {
    return await _executeWithLimit(_getRequestKey('group_leaders'), () async {
      final accountId = await _getCurrentAccountId();

      try {
        final response = await _makeRequest(
          '$_baseUrl/dashboard/progress/leader-group',
        );
        ...
        if (response.statusCode == 200) {
          ...
          try {
            ...
            try {
              final groupModel = GroupPositionModel.fromJson(
          ...
          print(
            '✅ Лидеры группы загружены и сохранены в SQLite: ${leaders.length} шт',
          );
          ...
          // Пробуем из SQLite
          final offlineLeaders = await _databaseFacade.getGroupLeaders(
            accountId,
          );
          if (offlineLeaders.isNotEmpty) {
            print(
              '📱 Используем лидеров группы из SQLite: ${offlineLeaders.length} шт',
            );
            return offlineLeaders;
          }

          throw Exception(
            'Failed to load group leaders: ${response.statusCode}',
          );
        }
      } catch (e) {
        ...
        if (cachedLeaders != null && cachedLeaders.isNotEmpty) {
          print(
            '💾 Используем лидеров группы из кэша: ${cachedLeaders.length} шт',
          );
        ...
        if (offlineLeaders.isNotEmpty) {
          print(
            '🗄️ Используем лидеров группы из SQLite: ${offlineLeaders.length} шт',
          );
          return offlineLeaders;
        }
  ...
  Future<List<LeaderboardUser>> getStreamLeaders(String token) async {
    return await _executeWithLimit(_getRequestKey('stream_leaders'), () async {
      final accountId = await _getCurrentAccountId();

      try {
        final response = await _makeRequest(
          '$_baseUrl/dashboard/progress/leader-stream',
        );
        ...
        if (response.statusCode == 200) {
          ...
          try {
            ...
            try {
              final streamModel = StreamPositionModel.fromJson(
                jsonDecode(response.body),
              );
          ...
          print(
            '✅ Лидеры потока загружены и сохранены в SQLite: ${leaders.length} шт',
          );
          ...
          // Пробуем из SQLite
          final offlineLeaders = await _databaseFacade.getStreamLeaders(
            accountId,
          );
          if (offlineLeaders.isNotEmpty) {
            print(
              '📱 Используем лидеров потока из SQLite: ${offlineLeaders.length} шт',
            );
            return offlineLeaders;
          }

          throw Exception(
            'Failed to load stream leaders: ${response.statusCode}',
          );
        }
      } catch (e) {
        ...
        if (cachedLeaders != null && cachedLeaders.isNotEmpty) {
          print(
            '💾 Используем лидеров потока из кэша: ${cachedLeaders.length} шт',
          );
          return cachedLeaders;
        }
        ...
        if (offlineLeaders.isNotEmpty) {
          print(
            '🗄️ Используем лидеров потока из SQLite: ${offlineLeaders.length} шт',
          );
          return offlineLeaders;
        }
  ...
  Future<List<FeedbackReview>> getFeedbackReview(String token) async {
    return await _executeWithLimit(_getRequestKey('feedback'), () async {
      final accountId = await _getCurrentAccountId();
      ...
      try {
        ...
        if (response.statusCode == 200) {
          ...
          final feedbacks = feedbackData
              .map((json) => FeedbackReview.fromJson(json))
              .toList();
          ...
          print(
            '✅ Отзывы загружены и сохранены в SQLite: ${feedbacks.length} шт',
          );
          ...
          // Пробуем из SQLite
          final offlineFeedbacks = await _databaseFacade.getFeedbacks(
            accountId,
          );
          if (offlineFeedbacks.isNotEmpty) {
            print(
              '📱 Используем отзывы из SQLite: ${offlineFeedbacks.length} шт',
            );
        ...
        // Пробуем из кэша
        final cachedFeedbacks = await _cacheRepository
            .get<List<FeedbackReview>>(
              CacheKeys.feedback,
              accountId: accountId,
            );
        ...
        if (offlineFeedbacks.isNotEmpty) {
          print(
            '🗄️ Используем отзывы из SQLite: ${offlineFeedbacks.length} шт',
          );
          return offlineFeedbacks;
        }
  ...
  Future<List<Exam>> getExams(String token) async {
    return await _executeWithLimit(_getRequestKey('exams'), () async {
      final accountId = await _getCurrentAccountId();
      ...
      try {
        ...
        if (response.statusCode == 200) {
          ...
          print(
            '✅ Экзамены загружены и сохранены в SQLite: ${exams.length} шт',
          );
          ...
          if (offlineExams.isNotEmpty) {
            print(
              '📱 Используем экзамены из SQLite: ${offlineExams.length} шт',
            );
            return offlineExams;
          }
  ...
  Future<List<Exam>> getFutureExams(String token) async {
    return await _executeWithLimit(_getRequestKey('future_exams'), () async {
      final accountId = await _getCurrentAccountId();
      ...
      try {
        ...
        if (response.statusCode == 200) {
          final responseData = jsonDecode(response.body);
          final futureExamsData = responseData is List
              ? responseData
              : <dynamic>[];
          ...
          print(
            '✅ Предстоящие экзамены загружены и сохранены в SQLite: ${exams.length} шт',
          );
          ...
          final offlineExams = (await _databaseFacade.getExams(
            accountId,
          )).where((exam) => exam.isFuture).toList();
          if (offlineExams.isNotEmpty) {
            print(
              '📱 Используем экзамены из SQLite: ${offlineExams.length} шт',
            );
          ...
          throw Exception(
            'Failed to load future exams: ${response.statusCode}',
          );
        }
      } catch (e) {
  ...
  Future<List<ActivityRecord>> getProgressActivity(String token) async {
    return await _executeWithLimit(_getRequestKey('activity'), () async {
      final accountId = await _getCurrentAccountId();

      try {
        final response = await _makeRequest(
          '$_baseUrl/dashboard/progress/activity',
        );
        ...
        if (response.statusCode == 200) {
          ...
          print(
            '✅ Активности загружены и сохранены в SQLite: ${activities.length} шт',
          );
          ...
          // Пробуем из SQLite
          final offlineActivities = await _databaseFacade.getActivities(
            accountId,
          );
          if (offlineActivities.isNotEmpty) {
            print(
              '📱 Используем активности из SQLite: ${offlineActivities.length} шт',
            );
        ...
        // Пробуем из кэша
        final cachedActivities = await _cacheRepository
            .get<List<ActivityRecord>>(
              CacheKeys.activity,
              accountId: accountId,
            );
        ...
        if (cachedActivities != null && cachedActivities.isNotEmpty) {
          print(
            '💾 Используем активности из кэша: ${cachedActivities.length} шт',
          );
        ...
        if (offlineActivities.isNotEmpty) {
          print(
            '🗄️ Используем активности из SQLite: ${offlineActivities.length} шт',
          );
          return offlineActivities;
        }
  ...
  Future<List<Homework>> getHomeworks(
    ...
    final params =
        '${page ?? ''}_${status ?? ''}_${groupId ?? ''}_${specId ?? ''}_${type ?? ''}';

    return await _executeWithLimit(_getRequestKey('homeworks', params), () async {
      final accountId = await _getCurrentAccountId();
      ...
      try {
        ...
        if (response.statusCode == 200) {
          ...
          final homeworks = homeworkData.map((json) {
            final typedJson = Map<String, dynamic>.from(json as Map);
            if (type != null) {
              typedJson['material_type'] = type;
            }
            return Homework.fromJson(typedJson);
          }).toList();
          ...
          print(
            '✅ ${type == 1 ? 'Лабораторные' : 'Домашние'} задания загружены и сохранены в SQLite: ${homeworks.length} шт',
          );
          ...
          if (offlineHomeworks.isNotEmpty) {
            print(
              '📱 Используем ${type == 1 ? 'лабораторные' : 'домашние'} задания из SQLite: ${offlineHomeworks.length} шт',
            );
        ...
        if (cachedHomeworks != null && cachedHomeworks.isNotEmpty) {
          print(
            '💾 Используем домашние задания из кэша: ${cachedHomeworks.length} шт',
          );
          ...
          if (status != null) {
            filtered = filtered
                .where((hw) => hw.getDisplayStatus() == status)
                .toList();
          }
        ...
        if (offlineHomeworks.isNotEmpty) {
          print(
            '🗄️ Используем домашние задания из SQLite: ${offlineHomeworks.length} шт',
          );
          return offlineHomeworks;
        }
        ...
        try {
          ...
          if (response.statusCode == 200) {
            ...
            final counters = counterData
                .map((json) => HomeworkCounter.fromJson(json))
                .toList();

            // Сохраняем в SQLite
            await _databaseFacade.saveHomeworkCounters(
              counters,
              accountId,
              type: type,
            );
            ...
            print(
              '✅ Счетчики ${type == 1 ? 'лабораторных' : 'домашних'} заданий загружены и сохранены в SQLite: ${counters.length} шт',
            );
            ...
            // Пробуем из SQLite
            final offlineCounters = await _databaseFacade.getHomeworkCounters(
              accountId,
              type: type,
            );
            if (offlineCounters.isNotEmpty) {
              print(
                '📱 Используем счетчики из SQLite: ${offlineCounters.length} шт',
              );
              return offlineCounters;
            }

            throw Exception(
              'Failed to load homework counters: ${response.statusCode}',
            );
          }
        } catch (e) {
          ...
          // Пробуем из кэша
          final cachedCounters = await _cacheRepository
              .get<List<HomeworkCounter>>(
                CacheKeys.homeworkCounters,
                accountId: accountId,
              );
          ...
          if (cachedCounters != null && cachedCounters.isNotEmpty) {
            print(
              '💾 Используем счетчики из кэша: ${cachedCounters.length} шт',
            );
          ...
          if (offlineCounters.isNotEmpty) {
            print(
              '🗄️ Используем счетчики из SQLite: ${offlineCounters.length} шт',
            );
            return offlineCounters;
          }
  ...
  Future<void> syncAllData(String token) async {
    ...
    try {
      ...
      await getSchedule(token, formatDate(monday), formatDate(sunday)).then((
        schedule,
      ) async {
      ...
      await Future.wait(
        [
          ...
          getHomeworks(token, type: 0).then((homeworks) async {
            ...
            print(
              '✅ Домашние задания синхронизированы: ${homeworks.length} шт',
            );
          }),
          ...
          getHomeworks(token, type: 1).then((homeworks) async {
            ...
            print(
              '✅ Лабораторные работы синхронизированы: ${homeworks.length} шт',
            );
          }),
          ...
          getHomeworkCounters(token, type: 1).then((counters) async {
            ...
            print(
              '✅ Счетчики лабораторных синхронизированы: ${counters.length} шт',
            );
          }),
        ...
        eagerError: false,
      ); // false - чтобы ошибка в одном не останавливала остальные
  ...
  Future<bool> validateToken(String token) async {
    return await _executeWithLimit(_getRequestKey('validate_token'), () async {
      try {
      ...
      final file = await DownloadService.downloadFile(
        ...
        onProgress: (received, total) {
          if (total != -1) {
            ...
            ); // TODO: допилить прогресс в UX - Ди.
  ...
  Future<File?> downloadStudentHomeworkFile(
    String token,
    Homework homework,
  ) async {
    try {
      if (homework.studentDownloadUrl == null ||
          homework.studentDownloadUrl!.isEmpty) {
        throw Exception('URL файла студенческой работы недоступен');
      }

      final String fileName =
          homework.safeStudentFilename ??
