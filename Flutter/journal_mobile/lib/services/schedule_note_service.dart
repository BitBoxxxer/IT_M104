...
class ScheduleNoteService {
  ...
  Future<void> checkAndTriggerReminders() async {
    ...
    final notesWithReminders = await _databaseFacade.getNotesWithReminders(
      _currentAccountId!,
    );
  ...
  Future<List<ScheduleNote>> getUpcomingReminders({int limit = 5}) async {
    ...
    return await _databaseFacade.getUpcomingReminders(
      _currentAccountId!,
      limit: limit,
    );
  }
  ...
  Future<List<ScheduleNote>> getNotesForDate(DateTime date) async {
    ...
    return await _databaseFacade.getScheduleNotesForDate(
      _currentAccountId!,
      date,
    );
  }
  ...
  Future<void> scheduleNoteRemindersForBackground() async {
    ...
    print(
      '🔔 Планирование напоминаний для ${notesWithReminders.length} заметок',
    );
  ...
  Future<void> _scheduleNoteReminder(ScheduleNote note) async {
    ...
    print(
      '📅 Напоминание запланировано на ${note.reminderTime} для заметки: ${note.noteText}',
    );
  }
  ...
  Future<int> saveNote({
    ...
    print(
      '✅ Заметка ${noteId != null ? 'обновлена' : 'сохранена'} с ID: $savedNoteId',
    );
    return savedNoteId;
  }
  ...
  Future<void> _cancelScheduledReminder(int noteId) async {
    try {
      await _safeNotificationService.notifications.cancel(noteId);
      // Cancel the previous ID scheme as well, for reminders scheduled by older builds.
  ...
  Future<void> updateNoteReminder(
    ...
    final note = await _databaseFacade.getScheduleNoteById(
      noteId,
      _currentAccountId!,
    );
    if (note == null) return;
  ...
  Future<void> deleteNote(int noteId) async {
    ...
    await _cancelScheduledReminder(noteId);
    await _safeNotificationService.deleteNotificationsForNote(noteId);
  ...
  Future<void> scheduleAllReminders() async {
    ...
    final allNotes = await _databaseFacade.getAllScheduleNotes(
      _currentAccountId!,
    );
    final now = DateTime.now();
  ...
  Future<void> checkOverdueReminders() async {
    ...
    for (final note in allNotes) {
      if (note.reminderEnabled &&
          ...
          note.reminderTime!.isAfter(now.subtract(Duration(days: 1)))) {
