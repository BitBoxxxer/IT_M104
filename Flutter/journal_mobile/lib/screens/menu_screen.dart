...
class _MainMenuScreenState extends State<MainMenuScreen> {
  ...
  void _onNotificationTap(Map<String, dynamic> payload) {
    if (!mounted || payload['type'] != 'new_mark') return;
  ...
  Widget _buildMainMenuScreen() {
    return SingleChildScrollView(child: _buildMainContent());
  }

  Widget _buildHomeworkScreen() {
    return Column(
      children: [
        AppBar(title: Text('Задания'), centerTitle: true),
        Expanded(
                    ...
                    Container(
                      ...
                      child: Row(
                        children: [
                          ...
                          Expanded(
                            child: Text(
                              'Офлайн-режим: доступны ранее сохраненные задания, скачивание файлов отключено',
                              style: TextStyle(
                                fontSize: 12,
                                color: Colors.orange.shade700,
                              ),
                    ...
                    Container(
                      ...
                      child: Row(
                        children: [
                          ...
                          Expanded(
                            child: Text(
                              'Домашние и лабораторные задания доступны в этом разделе',
                              style: TextStyle(
                                fontSize: 12,
                                color: Colors.orange.shade700,
                              ),
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton.icon(
                      ...
                      onPressed: () {
                        Navigator.of(
                          context,
                        ).push(MaterialPageRoute(builder: (_) => ExamScreen()));
                      },
                      style: ElevatedButton.styleFrom(
                        padding: EdgeInsets.symmetric(vertical: 16),
                      ),
  ...
  Widget _buildMarksAndScheduleScreen() {
    return Column(
      children: [
        AppBar(title: Text('Оценки и Расписание'), centerTitle: true),
        Expanded(
          child: Center(
            child: Column(
              mainAxisAlignment: MainAxisAlignment.center,
                  ...
                  child: ElevatedButton.icon(
                    ...
                    onPressed: () {
                      Navigator.of(context).push(
                        MaterialPageRoute(
                          builder: (_) =>
                              MarksAndProfileScreen(token: widget.token),
                        ),
                      );
                    },
                    style: ElevatedButton.styleFrom(
  ...
  Future<void> _syncAllData() async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'Все данные синхронизированы для offline использования! ✅',
          ),
  ...
  Map<String, double> _calculateAverages(List<Mark> marks) {
    ...
    return {
      ...
      'control': controlWorkCount > 0
          ? totalControlWorkMarks / controlWorkCount
          : 0.0,
      'lab': labWorkCount > 0 ? totalLabWorkMarks / labWorkCount : 0.0,
      'practical': practicalWorkCount > 0
          ? totalPracticalWorkMarks / practicalWorkCount
          : 0.0,
        ...
        return Stack(
          children: [
            IconButton(
              ...
              onPressed: () {
                Navigator.of(context).push(
                  MaterialPageRoute(builder: (_) => UserNotificationScreen()),
                );
              },
            ),
            if (unreadCount > 0)
        ...
        children: [
          ...
          FutureBuilder<Map<String, dynamic>>(
            ...
            builder: (context, snapshot) {
              ...
              final twelvePointCount =
                  marks.length - filteredMarksForAverages.length;
                            ...
                            builder: (context, snapshot) {
                              ...
                              return Column(
                                ...
                                children: [
                                  ...
                                  SizedBox(height: 8),
                                  ...reminders
                                      .map((note) => _buildReminderItem(note))
                                      .toList(),
                                ],
                              );
                            },
        ...
        child: Scaffold(
          ...
          appBar: AppBar(
            ...
            actions: <Widget>[
              ...
              IconButton(icon: Icon(Icons.menu), onPressed: _togglePanel),
            ],
          ),
          body: PageView(
                  ...
                  ListTile(
                    ...
                    onTap: () {
                      ...
                      Navigator.of(context).push(
                        MaterialPageRoute(
                          builder: (_) =>
                              FeedbackReviewScreen(token: widget.token),
                        ),
                      );
                    },
                  ),
  ...
  Widget _buildReminderItem(ScheduleNote note) {
    ...
    return Card(
      ...
      shape: RoundedRectangleBorder(
        ...
        side: BorderSide(
          color:
              note.noteColor?.withOpacity(0.3) ?? Colors.blue.withOpacity(0.3),
          width: 1,
        ),
      ),
      child: ListTile(
        ...
        subtitle: Row(
          children: [
            ...
            SizedBox(width: 4),
            Text(time, style: TextStyle(fontSize: 12, color: Colors.orange)),
          ],
        ),
          ...
          children: [
            Row(
              children: [
                ...
                SizedBox(width: 8),
                Expanded(
                  child: Text(note.noteText, style: TextStyle(fontSize: 16)),
                ),
              ],
            ),
            SizedBox(height: 16),
  ...
  String _getPositionSuffix(int position) {
    ...
    if (position % 10 >= 2 &&
        position % 10 <= 4 &&
        (position % 100 < 10 || position % 100 >= 20)) {
      return 'места';
    }
    return 'место';
  }
